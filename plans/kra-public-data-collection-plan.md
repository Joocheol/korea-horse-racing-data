...
- 개발계정 실측 한도: 필수 8개 API 모두 endpoint별 일일 3,000회
- 운영단계 상향은 8개 모두 심의승인이며 자동 전환으로 가정하지 않음
...
코드는 MIT License다. 2026-08-18 공공데이터포털 카탈로그를 직접 확인한 결과,
필수 8개 OpenAPI는 모두 무료이며 `이용허락범위 제한 없음`이다. 따라서 API
원응답 bulk와 정규화 사실값 공개에 대한 endpoint별 라이선스 게이트는 충족됐다.
데이터셋 ID·엔드포인트·포맷·확인일·카탈로그 URL은 `DATA_LICENSE.md`를 정본으로
사용하며, Release에도 KRA와 공공데이터포털을 출처로 표시한다.
...
이 단순화는 OpenAPI 8종에만 적용한다. KRA 웹사이트 스크래핑 원문은 별도 이용조건이
확인되기 전 공개 Release에서 제외하고 비공개 격리한다.
